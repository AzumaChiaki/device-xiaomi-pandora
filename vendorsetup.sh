...
# 关于橙狐变量详见 fox_12.1/vendor/recovery/orangefox_build_vars.txt
	...
	echo -e "\x1b[96mpandora: 开始添加OrangeFox Vars...\x1b[m"
	## 构建信息
	# 设置显示在关于页面里的维护人员名称
	export OF_MAINTAINER=ymdzq
	# 维护者补丁版本号（FOX_VERSION 已废弃）
	...
	# Keymaster unavailable in recovery; skip FBE to avoid boot hang
	...
	# 使不同销售区域的pandora机型都能通过机型断言
	# export TARGET_DEVICE_ALT="pandora"
	# 当刷机包包含机型白名单时，可强制声明支持机型（与TARGET_OTA_ASSERT_DEVICE冲突）
	# export FOX_TARGET_DEVICES="pandora"
	...
	export OF_DYNAMIC_FULL_SIZE=13421772800
	...
	F=$(find "device" -maxdepth 2 -name "pandora")
	# 修改启动画面背景色为#000000
	...
	echo -e "\x1b[96mpandora: 当你看到这个消息的时候，所有的OrangeFox Var已经添加完毕！\x1b[m"
